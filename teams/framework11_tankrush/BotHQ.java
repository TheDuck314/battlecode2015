...
public class BotHQ extends Bot {
    ...
    private static void directStrategySoldierRush() throws GameActionException {

        RobotType desiredBuilding = RobotType.HQ;
        ...
        makeTanks = true;
        if(numTankFactories >= 1 && rc.getTeamOre() >= 310) {
            makeSoldiers = true;
        }
        ...
        // Choose the rally point
        if(numTanks >= 9) attackMode = true;

        MapLocation rallyLoc = null;
        if (!attackMode) {
            rallyLoc = new MapLocation((3 * ourHQ.x + theirHQ.x) / 4, (3 * ourHQ.y + theirHQ.y) / 4);
        } else {
            MapLocation[] enemyTowers = rc.senseEnemyTowerLocations();
            if (enemyTowers.length == 0) {
                rallyLoc = theirHQ;
