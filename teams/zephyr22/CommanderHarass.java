...
public class CommanderHarass extends Bot {
    ...
    private static boolean canTakeMultipleEnemies(RobotInfo[] enemiesAttackingUs, int numEnemiesAttackingUs) {
        ...
        for (int i = 0; i < numEnemiesAttackingUs; i++) {
            ...
            damageTakenWhileKillingAllEnemies += (enemyDps - GameConstants.REGEN_RATE) * turnsToKillEnemy;
        }
    ...
    private static boolean debug_canTakeMultipleEnemies(RobotInfo[] enemiesAttackingUs, int numEnemiesAttackingUs) {
        // suppose we kill the enemies in the reverse order they appear in the list.
        // then we can figure out how much damage we will take during that process

        Debug.indicate("micro", 1, "");
        ...
        for (int i = 0; i < numEnemiesAttackingUs; i++) {
            ...
            Debug.indicateAppend("micro", 1, "" + turnsToKillEnemy + " turns to kill " + enemy.health + " " + enemy.type.toString() + ", damage = " + damageTakenWhileKillingAllEnemies + ";");
        }
    ...
    private static boolean doMicro(RobotInfo[] nearbyEnemies, boolean shadowEnemyHarassers) throws GameActionException {
        ...
        debug_canTakeMultipleEnemies(enemiesAttackingUs, numEnemiesAttackingUs);
    ...
    private static void attackAndTryToApproach(MapLocation loc, RobotInfo[] nearbyEnemies) throws GameActionException {
        if (rc.isWeaponReady()) {
            rc.attackLocation(loc);
        }

        if (rc.isCoreReady()) {
            tryMoveTowardLocationSafely(loc, nearbyEnemies);
        }
    }
