...
public class Bfs extends Bot {
    ...
    public static void reinitQueue(MapLocation dest) {
        if (locQueue == null) {
            locQueue = new MapLocation[2 * GameConstants.MAP_MAX_WIDTH * 2 * GameConstants.MAP_MAX_HEIGHT];
            wasQueued = new boolean[2 * GameConstants.MAP_MAX_WIDTH][2 * GameConstants.MAP_MAX_HEIGHT];
            ...
            minX = ourHQ.x - GameConstants.MAP_MAX_WIDTH + 1;
            maxX = ourHQ.x + GameConstants.MAP_MAX_WIDTH - 1;
            minY = ourHQ.y - GameConstants.MAP_MAX_HEIGHT + 1;
            maxY = ourHQ.y + GameConstants.MAP_MAX_HEIGHT - 1;
        ...
        wasQueued = new boolean[2 * GameConstants.MAP_MAX_WIDTH][2 * GameConstants.MAP_MAX_HEIGHT];
        paths = new Direction[2 * GameConstants.MAP_MAX_WIDTH][2 * GameConstants.MAP_MAX_HEIGHT];
            ...
            for (int i = 8; i-- > 0;) {
                ...
                if (x >= minX && y >= minY && x <= maxX && y <= maxY && !wasQueued[x - minX][y - minY]) {
                    ...
                    if (terrain != TerrainTile.VOID && terrain != TerrainTile.OFF_MAP) {
                        ...
                        // System.out.println("go from " + newLoc.toString() + " to " + loc.toString());
