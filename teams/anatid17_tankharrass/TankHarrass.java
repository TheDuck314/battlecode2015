package anatid17_tankharrass;

import battlecode.common.*;

class SafetyPolicyTankHarrass extends Bot implements NavSafetyPolicy {
    RobotInfo[] nearbyEnemies;

    public SafetyPolicyTankHarrass(RobotInfo[] nearbyEnemies) {
        this.nearbyEnemies = nearbyEnemies;
    }

    public boolean isSafeToMoveTo(MapLocation loc) {
        return TankHarrass.isSafeToMoveTo(loc, nearbyEnemies);
    }
}

public class TankHarrass extends Bot {

    private static MapLocation[] enemyTowers;

    private static boolean canWin1v1(RobotInfo enemy) {
        int numAttacksAfterFirstToKillEnemy = (int) ((enemy.health - 0.001) / RobotType.TANK.attackPower);
        int turnsTillWeCanAttack;
        int effectiveAttackDelay;
        if (rc.getSupplyLevel() > 0) {
            turnsTillWeCanAttack = (int) rc.getWeaponDelay();
            effectiveAttackDelay = RobotType.TANK.attackDelay;
        } else {
            turnsTillWeCanAttack = (int) Math.max(0, (rc.getWeaponDelay() - 0.5) / 0.5);
            effectiveAttackDelay = 2 * RobotType.TANK.attackDelay;
        }
    ...
    public static boolean canWin1v1AfterMovingTo(MapLocation loc, RobotInfo enemy) {
        double loadingDelay = RobotType.TANK.loadingDelay;
    ...
    public static boolean isSafeToMoveTo(MapLocation loc, RobotInfo[] nearbyEnemies) {
        ...
        return TankHarrass.canWin1v1AfterMovingTo(loc, loneAttacker);
    }
    ...
    private static boolean tryMoveTowardUndefendedHelplessEnemy() throws GameActionException {
        ...
        for (RobotInfo enemy : nearbyEnemies) {
            if (enemy.type == RobotType.BEAVER || enemy.type == RobotType.MINER) {
                ...
                while (loc.distanceSquaredTo(enemy.location) > RobotType.TANK.attackRadiusSquared) {
                    Direction dir = loc.directionTo(enemy.location);
                    MapLocation newLoc = loc.add(dir);
                    if (rc.isPathable(RobotType.TANK, newLoc) && isSafeToMoveTo(newLoc, nearbyEnemies)) {
                        loc = newLoc;
                    } else {
                        canReach = false;
                        break;
    ...
    private static boolean tryStickToEnemyHarrassers() throws GameActionException {
        RobotInfo[] nearbyEnemies = rc.senseNearbyRobots(80, them);

        RobotInfo closestEnemyDrone = null;
        int minDistSq = 999999;
        for (RobotInfo enemy : nearbyEnemies) {
            if (enemy.type == RobotType.DRONE || enemy.type == RobotType.SOLDIER || enemy.type == RobotType.TANK) {
...
//                if (tryStickToEnemyHarrassers()) return;
//            }
            if (tryMoveTowardUndefendedHelplessEnemy()) return;

            RobotInfo[] nearbyEnemies = rc.senseNearbyRobots(35, them);
            NavSafetyPolicy safetyPolicy = new SafetyPolicyTankHarrass(nearbyEnemies);
