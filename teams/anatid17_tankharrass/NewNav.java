...
public class NewNav extends Bot {
    ...
    private static boolean move(Direction dir) throws GameActionException {
        if (dir.isDiagonal() && rc.getType().movementDelay == 1 && rc.getCoreDelay() >= 0.6) {
            // often it's safer to wait here for our delay to go down rather than be stuck
            // unable to move on the adjacent square
            return false;
        }
        rc.move(dir);
        return true;
