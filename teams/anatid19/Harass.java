        ...
        if (numEnemiesAttackingUs > 0) {
            ...
            if (rc.isCoreReady()) { // all remaining possible actions are movements
                ...
                if (closestEnemy != null && rc.getType().attackRadiusSquared >= closestEnemy.type.attackRadiusSquared && closestEnemy.type != RobotType.MISSILE) {
                    ...
                    /*
                     * int maxEnemyExposure = numAlliesFightingEnemy; if (maxEnemyExposure == 0) { boolean canWin =
                     * canWin1v1AfterMovingTo(here.add(here.directionTo(closestEnemy.location)), closestEnemy); Debug.indicate("micro", 1, "canWin against " +
                     * closestEnemy.location.toString() + " = " + canWin); if (canWin) { maxEnemyExposure = 1; } } if (maxEnemyExposure > 0) { if
                     * (tryMoveTowardLocationWithMaxEnemyExposure(closestEnemy.location, maxEnemyExposure, nearbyEnemies)) { Debug.indicate("micro", 0,
                     * "moved to engage " + closestEnemy.location.toString() + "; maxEnemyExposure = " + maxEnemyExposure); return true; } else {
