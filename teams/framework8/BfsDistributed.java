...
public class BfsDistributed extends Bot {
    ...
    public static Direction readResult(MapLocation start, MapLocation dest, int minInitRound) throws GameActionException {
        ...
        Debug.indicate("bfsdist", 1, "my dest is " + dest.toString() + ". search dest is " + mapLocationFromInt(rc.readBroadcast(CHANNEL_DEST)).toString()
                + ". got result from round " + publishedRound);
    ...
    public static void reinitQueue(MapLocation dest) throws GameActionException {
        ...
        Debug.indicate("bfsdist", 1, "reinited queue with dest = " + dest.toString());
    }

    private static boolean isDefinitelyOnMap(MapLocation loc) {
        TerrainTile tt = rc.senseTerrainTile(loc);
        return tt == TerrainTile.NORMAL || tt == TerrainTile.VOID;
    }

    public static void work(MapLocation dest) throws GameActionException {
        if (dirs == null) init();

        if (rc.readBroadcast(CHANNEL_DEST) != intFromMapLocation(dest)) {
            reinitQueue(dest);
            return;
        }
        ...
        // System.out.println("starting work. initRound = " + initRound + ", locQueueHead = " + locQueueHead + ", locQueueTail = " + locQueueTail);
        Debug.indicate("bfsdist", 0, "dest = " + dest.toString() + ", locQueueHead = " + locQueueHead + "; " + mapMinX + " <= x <= " + mapMaxX + "; " + mapMinY
                + " <= y <= " + mapMaxY);

        int initialLocQueueTail = locQueueTail;
        boolean[][] wasQueued = new boolean[GameConstants.MAP_MAX_WIDTH][GameConstants.MAP_MAX_HEIGHT];

        int bytecodeLimit = rc.getSupplyLevel() > rc.getType().supplyUpkeep ? 8500 : 3500;

        while (locQueueHead != initialLocQueueTail && Clock.getBytecodeNum() < bytecodeLimit) {
            // pop a location from the queue
            MapLocation loc = popFromQueue();
            // System.out.println("popped " + loc.toString());
            ...
            for (int i = 8; i-- > 0;) {
                ...
                if (x >= minX && y >= minY && x <= maxX && y <= maxY) {
                    if (x >= mapMinX && y >= mapMinY && x <= mapMaxX && y <= mapMaxY) {
                        if (!wasQueued[x - minX][y - minY]) {
                            MapLocation newLoc = new MapLocation(x, y);
                            if (!checkWasQueued(newLoc)) {
                                TerrainTile terrain = rc.senseTerrainTile(newLoc);
                                if (terrain == TerrainTile.OFF_MAP) {
                                    if (isDefinitelyOnMap(newLoc.add(1, 0))) mapMinX = x + 1;
                                    if (isDefinitelyOnMap(newLoc.add(-1, 0))) mapMaxX = x - 1;
                                    if (isDefinitelyOnMap(newLoc.add(0, 1))) mapMinY = y + 1;
                                    if (isDefinitelyOnMap(newLoc.add(0, -1))) mapMaxY = y - 1;
                                    continue;
                                }
                                if (terrain != TerrainTile.VOID && terrain != TerrainTile.OFF_MAP) {
                                    // System.out.println("path: " + newLoc.toString() + " - " + dirs[i].toString());
        ...
        // System.out.println("ending work. locQueueHead = " + locQueueHead + ", locQueueTail = " + locQueueTail);
    }
}
