    ...
    private static boolean tryFleeMissiles(RobotInfo[] nearbyEnemies) throws GameActionException {
        ...
        for (RobotInfo enemy : nearbyEnemies) {
            if (enemy.type == RobotType.MISSILE && here.distanceSquaredTo(enemy.location) <= 8) {
                // check if there are soldiers in range that can shoot down this missile
                if (rc.getWeaponDelay() >= 1) {
                    needToFlee = true;
                    break;
                } else {
                    int numShotsAvailable = 1;
                    RobotInfo[] nearbyAllies = rc.senseNearbyRobots(enemy.location, RobotType.SOLDIER.attackRadiusSquared, us);
                    for (RobotInfo ally : nearbyAllies) {
                        if (ally.type == RobotType.SOLDIER) {
                            if (ally.weaponDelay < 1.5 || (ally.weaponDelay < 2 && ally.supplyLevel > 0)) {
                                numShotsAvailable++;
                            }
                        }
                    }
                    if (numShotsAvailable >= enemy.health) {
