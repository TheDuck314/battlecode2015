...
public class Harass extends Bot {
    ...
    public static void doHarass() throws GameActionException {
        ...
        if (Clock.getRoundNum() % 3 == 0) {
            int numNearbyEnemyMiners = 0;
            for (RobotInfo enemy : nearbyEnemies) {
                if (enemy.type == RobotType.MINER) {
                    numNearbyEnemyMiners++;
                }
            }
            if (numNearbyEnemyMiners > MessageBoard.MOST_MINERS_IN_SIGHT.readInt()) {
                MessageBoard.MOST_MINERS_IN_SIGHT.writeInt(numNearbyEnemyMiners);
                MessageBoard.LEAD_HARASSER_ID.writeInt(rc.getID());
            }
        } else if (Clock.getRoundNum() % 3 == 1) {
            leadHarasserId = MessageBoard.LEAD_HARASSER_ID.readInt();
        } else {
            MessageBoard.MOST_MINERS_IN_SIGHT.writeInt(0);
            MessageBoard.LEAD_HARASSER_ID.writeInt(0);
        }
        ...
        if (rc.isCoreReady()) {
            ...
            if (leadHarasserId != rc.getID() && rc.canSenseRobot(leadHarasserId)) {
                RobotInfo leadHarasser = rc.senseRobot(leadHarasserId);
                Nav.goTo(leadHarasser.location, safetyPolicy);
            } else {
