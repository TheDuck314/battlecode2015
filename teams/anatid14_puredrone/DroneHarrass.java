...
public class DroneHarrass extends Bot {
    ...
    // Normally, just bug around units.
    //
    // If you see a miner or beaver and can engage it without getting hit, do so.
    // Next iteration will have to deal with miners and beavers fighting back.
    //
    // If anyone else engages you, run.
    ...
    private static boolean tryToAttack() throws GameActionException {
        // TODO: prioritize targets by health
        ...
        for (RobotInfo enemy : nearbyEnemies) {
            if (enemy.type == RobotType.BEAVER || enemy.type == RobotType.MINER) {
                ...
                while (loc.distanceSquaredTo(enemy.location) > RobotType.DRONE.attackRadiusSquared) {
                    ...
                    if (rc.isPathable(RobotType.DRONE, newLoc) && DroneNav.isSafe(newLoc)) {
                        loc = newLoc;
                    } else {
                        canReach = false;
                        break;
                ...
                if (canReach) {
                    rc.move(here.directionTo(enemy.location));
                    return true;
                }
            }
        }
