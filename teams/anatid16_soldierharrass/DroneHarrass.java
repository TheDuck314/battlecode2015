...
public class DroneHarrass extends Bot {
    ...
    public static void doHarrass() throws GameActionException {
        ...
        if (attackableEnemies.length > 0) {
            debug_canWin1v1(attackableEnemies[0]);
        } else {
            RobotInfo[] moveAttackableEnemies = rc.senseNearbyRobots(13, them);
            if (moveAttackableEnemies.length > 0) {
                debug_canWin1v1AfterMovingTo(here.add(here.directionTo(moveAttackableEnemies[0].location)), moveAttackableEnemies[0]);
            }
        }
