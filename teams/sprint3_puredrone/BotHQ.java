...
public class BotHQ extends Bot {
    ...
    static boolean finalAttack = false;
    static MapLocation lastRallyLoc = null;
    ...
    private static void directStrategyDroneContain() throws GameActionException {
        ...
        MapLocation rallyLoc = null;
        double attackDifficulty = 0.0;
        if (!attackMode) {
            rallyLoc = new MapLocation((ourHQ.x + theirHQ.x) / 2, (ourHQ.y + theirHQ.y) / 2);
        } else {
            if (enemyTowers.length == 0) {
                rallyLoc = theirHQ;
                attackDifficulty = 1.5;
            } else {
                for (MapLocation tower : enemyTowers) {
                    if (rallyLoc == null || ourHQ.distanceSquaredTo(tower) < ourHQ.distanceSquaredTo(rallyLoc)) {
                        rallyLoc = tower;
                ...
                for(MapLocation tower : enemyTowers) {
                    if(tower.distanceSquaredTo(rallyLoc) <= 24) {
                        attackDifficulty += 1.0;
                    }
                }
            }
        }
        ...
        if(!rallyLoc.equals(lastRallyLoc)) {
            finalAttack = false;
        }
        if(numDrones < 18 * attackDifficulty) {
            finalAttack = false;
        }
        int numAlliedTowers = rc.senseTowerLocations().length;
        if(Clock.getRoundNum() > 1925 && enemyTowers.length >= numAlliedTowers) {
            finalAttack = true;
        }
        if(numDrones > 25 * attackDifficulty) {
            finalAttack = true;
        }
        MessageBoard.FINAL_ATTACK_SIGNAL.writeBoolean(finalAttack);
        
        lastRallyLoc = rallyLoc;
    }
